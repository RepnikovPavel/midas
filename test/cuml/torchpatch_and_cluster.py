...
import torch
import cupy
from patchworktorch import patchworktorch_class
...
if __name__ == "__main__":
    ...
    gfilter = patchworktorch_class(
        params={
            'verbose':False,
            'sensor_height':-100.0,
            'max_range':80.0,
            'min_range':2.7,
            'adaptive_seed_selection_margin':-1000.0
        }
    )
    ...
    if len(dataset) > 0:
        for sweep_idx in range(len(dataset)):
            ...
            for snapshot_idx in tqdm(range(len(sweep))):
                ...
                print('type pts_ego',pts_ego.dtype)
                print('shape pts_ego',pts_ego.shape)
                torch.cuda.synchronize()
                ground_filter_labels = gfilter.forward(
                    torch.from_numpy(pts_ego).contiguous().to('cuda'),
                ).cpu().detach().numpy()

                torch.cuda.synchronize()
                groud_points_maks = ground_filter_labels==0
                not_ground_points_mask = np.logical_not(groud_points_maks)
                ground_points = pts_ego[groud_points_maks]
                not_ground_points = pts_ego[not_ground_points_mask]

                cudf_ = cudf.DataFrame()
                cudf_['x'] = not_ground_points[:,0]
                cudf_['y'] = not_ground_points[:,1]
                cudf_['z'] = not_ground_points[:,2]
                dbscan_float = DBSCAN(eps=0.64, min_samples=20)
                torch.cuda.synchronize()
                ...
                all_pts_colors = np.concatenate((ground_points_colors,cluster_colors_np))
                ...
                vis.update(
                    [
                        {
                            'plot_type':'points',
                            'data':all_pts_for_vis,
                            'colors':all_pts_colors,
                        },
                        {
                            'plot_type':'boxes',
                            'data':boxes_3d_ego
