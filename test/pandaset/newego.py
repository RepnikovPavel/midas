...
def load_pandaset_data(root):
    ...
    # Load LiDAR Poses (Vehicle in World)
    ...
    for cam in cam_names:
        cameras_data[cam] = {
            'images': [], 
            'intr': CALIBRATIONS_DATA.get(cam, {}).get('intrinsic'),
            'transforms': [] # List to store precomputed T_ego_to_cam
        }
    ...
    for i, pkl_file in tqdm(enumerate(pkl_files), total=len(pkl_files)):
        ...
        # Transform Points to Ego-Vispy Frame
        ...
        # Process Cameras for this frame
        for cam in cam_names:
            # Load Image
            ...
            # Precompute Transform: Ego-Vispy -> Camera
            # Chain: Ego -> Vehicle -> World -> Camera
            # T_ego_to_cam = T_cam_world @ T_world_veh @ T_ego_to_veh
            if i < len(cameras_data[cam]['poses']):
                ...
                # T_ego_to_veh is T_AXIS_INV_4x4
                T_ego_to_cam = T_cam_world @ T_world_veh @ T_AXIS_INV_4x4
                cameras_data[cam]['transforms'].append(T_ego_to_cam)
            else:
                cameras_data[cam]['transforms'].append(np.eye(4))
    ...
    for i, cam_name in enumerate(cam_names):
        if cam_name not in cameras_data:
            ...
            if pil_img is None:
                ...
                img = pil_img.copy() # Work on copy
                
                # Get Precomputed Transform (Ego -> Cam)
                T_ego_to_cam = cam_block['transforms'][frame_idx]
                
                # Project Points
                # P_cam = T_ego_to_cam @ P_ego
                points_cam = (T_ego_to_cam[:3, :3] @ pts_ego.T + T_ego_to_cam[:3, 3:4]).T
                ...
                if boxes_ego is not None and boxes_ego.shape[0] > 0:
                    ...
                    for k in range(boxes_ego.shape[0]):
                        ...
                        # Transform to Cam using precomputed matrix
                        corners_cam = (T_ego_to_cam[:3, :3] @ corners_ego.T + T_ego_to_cam[:3, 3:4]).T
                        ...
                        top_center_cam = T_ego_to_cam[:3, :3] @ top_center_ego + T_ego_to_cam[:3, 3]
...
if __name__ == "__main__":
    ...
    def update_loop(event):
        ...
        # Pass Ego-Vispy data to drawing function (no pose needed anymore)
