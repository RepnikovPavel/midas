...
# --- Numba-optimized geometry calculations (без изменений) ---
...
class LidarVisualizer:
    def __init__(self, title="LiDAR 3D View"):
        ...
        # --- ДОБАВЛЕНО: Visual для выпуклых оболочек ---
        # shading=None отключает освещение, цвет будет чистым (flat shading)
        self.hull_mesh = visuals.Mesh(parent=self.view.scene, shading=None)
    ...
    def update(self, data_list):
        ...
        for item in data_list:
            ...
            # Пропускаем пустые данные (кроме hulls, там список проверяется ниже)
            if data is None:
                continue
            ...
            elif plot_type == 'hulls':
                # data - это список кортежей (vertices, faces)
                if isinstance(data, list):
                    for verts, faces in data:
                        if verts.shape[0] >= 3: # Минимум треугольник
                            hulls_accum.append((verts, faces))
                    
                    c = item.get('color', (1.0, 0.5, 0.0, 0.4))
                    hull_colors_accum.append(c)
        ...
        if len(boxes_accum) > 0:
            ...
            line_colors = np.repeat(all_box_colors, 24, axis=0)
            self.lines.set_data(line_pos, color=line_colors)
            self.arrows.set_data(arrow_pos, color='red')
        else:
            self.lines.set_data(np.zeros((0, 3)))
            self.arrows.set_data(np.zeros((0, 3)))

        # --- Отрисовка hulls (ИСПРАВЛЕНО) ---
        if len(hulls_accum) > 0:
            all_verts = []
            all_faces = []
            idx_offset = 0
            
            for verts, faces in hulls_accum:
                all_verts.append(verts)
                # Сдвигаем индексы граней
                all_faces.append(faces + idx_offset)
                idx_offset += verts.shape[0]
            
            if idx_offset > 0:
                V = np.vstack(all_verts).astype(np.float32)
                F = np.vstack(all_faces).astype(np.int32)
                mesh_color = hull_colors_accum[0] if hull_colors_accum else (1, 0.5, 0, 0.4)
                
                self.hull_mesh.set_data(vertices=V, faces=F, color=mesh_color)
                self.hull_mesh.visible = True
            else:
                # Данных нет - скрываем
                self.hull_mesh.visible = False
        else:
            # Данных нет - скрываем
            self.hull_mesh.visible = False